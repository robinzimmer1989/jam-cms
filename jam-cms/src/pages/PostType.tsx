...
const PostType = (props: any) => {
    ...
    if (taxonomies && taxonomies.length > 0) {
        // @ts-expect-error ts-migrate(2345) FIXME: Argument of type '"" | { taxonomy: any; term: Sele... Remove this comment to see the full error message
        ...
        <CmsLayout pageTitle={config?.fields?.postTypes?.[postTypeID]?.title || postType?.title}>
          ...
          {visiblePosts && (<Space direction="vertical" size={8}>
              {filter === 'all' && postTypeID !== 'page' ? (<DragDropContext onDragEnd={handleDragEnd}>
                ...
                </DragDropContext>) : (visiblePosts.map((o: any) => renderPost(o, 0)))}
            </Space>)}
        </CmsLayout>
    );
};
